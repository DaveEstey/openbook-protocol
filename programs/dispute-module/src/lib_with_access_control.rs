...
/// DAO Authority State - stores authorized dispute resolvers
#[account]
...
pub mod dispute_module {
    ...
    /// Initialize DAO authority (one-time setup)
    ...
    /// Resolve dispute (DAO only - access controlled)
...
#[derive(Accounts)]
pub struct InitializeDaoAuthority<'info> {
    #[account(
        init,
        payer = payer,
        space = DaoAuthority::LEN,
...
pub struct ResolveDispute<'info> {
    ...
    /// DAO authority - SECURITY: Only authorized DAO can resolve
    #[account(
        seeds = [b"dao_authority"],
